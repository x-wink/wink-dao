...
describe('连接池和事务逻辑测试', () => {
    ...
    describe('事务完整性测试', () => {
        it('事务提交应正确保存所有操作', async () => {
            await dao.beginTransaction();

            try {
                const id1 = await dao.insert({
                    table: 'test_connection_pool',
                    data: [{ name: 'tx_user1', value: 100 }],
                });

                const id2 = await dao.insert({
                    table: 'test_connection_pool',
                    data: [{ name: 'tx_user2', value: 200 }],
                });
                ...
                // 验证数据已提交
                const record1 = await dao.get<{ value: number }>({
                    table: 'test_connection_pool',
                    where: { id: id1 },
                });
                const record2 = await dao.get<{ value: number }>({
                    table: 'test_connection_pool',
                    where: { id: id2 },
                });

                expect(record1?.value).toBe(150);
        ...
        it('事务回滚应撤销所有操作', async () => {
            await dao.beginTransaction();

            try {
                await dao.insert({
                    table: 'test_connection_pool',
                    data: [{ name: 'rollback_test', value: 999 }],
                });
        ...
        it('事务中发生错误应能正确回滚', async () => {
            const initialCount = await dao.count({ table: 'test_connection_pool' });
            ...
            try {
                await dao.insert({
                    table: 'test_connection_pool',
                    data: [{ name: 'error_test', value: 100 }],
                });
            ...
            // 验证数据未提交
            const finalCount = await dao.count({ table: 'test_connection_pool' });
        ...
        it('不能重复开启事务', async () => {
            ...
            try {
                await expect(dao.beginTransaction()).rejects.toThrow('不能重复开启事务');
                await dao.rollback();
            } catch (e) {
                await dao.rollback();
                throw e;
        ...
        it('未开启事务时提交应抛出错误', async () => {
            await expect(dao.commit()).rejects.toThrow('还没开启事务');
        });

        it('未开启事务时回滚应抛出错误', async () => {
            // 应该静默处理
            await expect(dao.rollback()).rejects.toThrow('还没开启事务');
        });

        it('commit失败后连接应正确释放', async () => {
            await dao.beginTransaction();
    ...
    describe('连接管理测试', () => {
        ...
        it('close时如果有未完成的事务应自动回滚', async () => {
            const tempDao = useDao({ config, debug: false });

            await tempDao.beginTransaction();
            await tempDao.insert({
                table: 'test_connection_pool',
    ...
    describe('数据一致性测试', () => {
        ...
        it('事务保证数据一致性', async () => {
            ...
            const id1 = await dao.insert({
                table: 'test_connection_pool',
                data: [{ name: 'account1', value: 100 }],
            });

            const id2 = await dao.insert({
                table: 'test_connection_pool',
                data: [{ name: 'account2', value: 100 }],
            });
            ...
            try {
                // 从account1扣除50
                const account1 = await dao.get<{ value: number }>({
                    table: 'test_connection_pool',
                    where: { id: id1 },
                });
                ...
                await dao.update({
                    table: 'test_connection_pool',
                    where: { id: id1 },
                    data: { value: account1!.value - 50 },
                });

                // 给account2增加50
                const account2 = await dao.get<{ value: number }>({
                    table: 'test_connection_pool',
                    where: { id: id2 },
                });
                ...
                await dao.update({
                    table: 'test_connection_pool',
                    where: { id: id2 },
                    data: { value: account2!.value + 50 },
                });

                await dao.commit();

                // 验证转账成功
                const finalAccount1 = await dao.get<{ value: number }>({
                    table: 'test_connection_pool',
                    where: { id: id1 },
                });
                const finalAccount2 = await dao.get<{ value: number }>({
                    table: 'test_connection_pool',
                    where: { id: id2 },
                });

                expect(finalAccount1?.value).toBe(50);
                expect(finalAccount2?.value).toBe(150);
            } catch (e) {
                await dao.rollback();
                throw e;
            }
